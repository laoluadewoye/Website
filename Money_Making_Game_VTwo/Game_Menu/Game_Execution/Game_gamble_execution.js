...
function boxing() {
  ...
  if (fighter == winner) {
    ...
    if (winner == 1) {
      alert(
        "You and " + team1 + " others won the grand pot of $" + groupPot + "!"
      );
      yourShare = Math.ceil(groupPot / team1);
    } else if (winner == 2) {
      alert(
        "You and " + team2 + " others won the grand pot of $" + groupPot + "!"
      );
    ...
    alert("Your boxer has unfortunately lost. You lose your contribution.");
    onHand = onHand - box;
  }
}
...
function bigone() {
  ...
  if (belowZero(one) != false) {
    ...
    if (one >= onHand * 0.75) {
      //75% min check
      let bigChance = Math.floor(Math.random() * 6);
